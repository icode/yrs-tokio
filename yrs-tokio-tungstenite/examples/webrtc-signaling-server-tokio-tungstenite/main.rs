...
use yrs_tokio_tungstenite::{YrsSignalStream, YrsSink};
...
async fn main() {
    ...
    while let Ok((stream, peer_addr)) = listener.accept().await {
        ...
        tokio::spawn(async move {
            if let Err(e) = handle_connection(stream, signaling_clone).await {
                eprintln!("连接处理失败: {}", e);
            } else {
                println!("连接处理完毕");
            }
...
async fn handle_connection(
    ...
    let is_websocket_upgrade = request_str.contains("Upgrade: websocket")
        ...
        && request_str.contains("GET /signaling HTTP");

    if is_websocket_upgrade {
        println!("检测到 WebSocket 请求，尝试握手...");
        let ws_stream = accept_async(stream).await?;
        println!("WebSocket 握手成功");
        peer(ws_stream, svc).await;
        Ok(())
    } else {
...
async fn peer<T>(ws: tokio_tungstenite::WebSocketStream<T>, svc: SignalingService)
where
